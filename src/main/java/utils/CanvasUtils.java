...
public class CanvasUtils {
	private final static int 	DEFAUL_BORDER_WIDTH = 0;
	...
	private static GVector2f calcPosition(GVector2f position, GVector2f size, Position type){
		switch(type){
			case CENTERED: return position.sub(size.div(2));
			case DOWN_RIGHT: return position.sub(size);
	...
	public static void drawRectCentered(Graphics2D g2, GVector2f pos, GVector2f size){
		drawRectCentered(g2, pos, size, DEFAULT_COLOR, DEFAUL_BORDER_WIDTH);
	}
	public static void drawRectCentered(Graphics2D g2, GVector2f pos, GVector2f size, Color color){
		drawRectCentered(g2, pos, size, color, DEFAUL_BORDER_WIDTH);
	}
