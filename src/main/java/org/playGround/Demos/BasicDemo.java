...
import org.playGround.Misc.map.SimpleMap_old;
...
public class BasicDemo extends GameState {
    ...
    public void input() {
        ...
        if (Input.isKeyDown(Keys.A)) { player.x -= playerSpeed; }
        if (Input.isKeyDown(Keys.D)) { player.x += playerSpeed; }
        if (Input.isKeyDown(Keys.S)) { player.y += playerSpeed; }
        if (Input.isKeyDown(Keys.W)) { player.y -= playerSpeed; }
    }
}
